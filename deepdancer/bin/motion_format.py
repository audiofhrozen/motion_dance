...
def format_motion_audio(filename, config, snr=None, noise='white', align=0):
    # TODO(nelson): Change configs and test differents
    ...
    silence_pos = np.ones((config['silence'] * config['fps'],
                           position_data.shape[1]), dtype=np.float32) * position_data[0, :]
...
def motionread(filename, extension, rottype='euler', joints=None, translation=False):
    if extension == 'htr':
        ...
        raise NotImplementedError(
            'The {} extension file is not implemented, Please, format the file in HTR'.format(extension))
    if translation:
...
def render_motion(motion, rottype='euler', Translation=True, scale=1):
    ...
    for i in range(len(JOINTS)):
        ...
        for j in range(num_frames):
            if rottype == 'euler':
                axis_motion[j, i * 3 + dims:(i + 1) * 3 + dims] = np.degrees(
                    motion[j, i * 3 + dims:(i + 1) * 3 + dims])  # rad2deg
            elif rottype == 'quat':
                axis_motion[j, i * 3 + dims:(i + 1) * 3 + dims] = np.degrees(
                    np.asarray([quat2euler(motion[j, i * 4 + dims:(i + 1) * 4 + dims])]))
            else:
                logging.error('Incorrect type of rotation')
                raise TypeError()
    return axis_motion
