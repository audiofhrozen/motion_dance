...
from python_utils.maths import deg2rad
...
def calculate_minmax(fileslist):
  ...
  for item in fileslist:
    ...
    for i in range(len(gnrl_conf.parts)):
      ...
      if i == 0:
        array_data = array_data[:,1:4] / Configs.pos_scale # Scaled Position Translation to meters
        ...
        for k in range(num_frames):   # Quaternion Margin Normalization
          new_array[k] = euler2quat(array_data[k,0],array_data[k,1],array_data[k,2])
          if gnrl_conf.rot_type=='euler':
            array_data[k] = quat2euler(new_array[k])
        if gnrl_conf.rot_type=='euler':
          tmp_minmax[i*3:i*3+3, 0] =  np.amin(array_data, axis = 0) # minimum
          tmp_minmax[i*3:i*3+3, 1] =  np.amax(array_data, axis = 0) # maximum                    
...
def format_data(filename, snr, Configs):
  ...
  for idx in range(len(Configs.parts)):
    ...
    array_data = np.loadtxt(array_data, delimiter='\t', dtype=np.float32)
    if idx == 0:
      array_data = array_data[:,1:4] / Configs.pos_scale # Scaled Position Translation 
      init_trans = np.mean(array_data[0:31], axis=0)
      array_data -= init_trans
      ...
      for k in range(num_frames):   # Quaternion Margin Normalization
        ...
        if Configs.rot_type=='euler':
          array_data[k] = quat2euler(new_array[k])
      if Configs.rot_type=='euler':
        position_data[:, idx*3:idx*3+3] =  array_data                  
