...
def add_noise(_AUDIO, _NOISE='clean', _SNR=None):
    ...
    if not ((_NOISE is None) or (_SNR is None) or (_NOISE.lower() == 'clean')):
        color = ['white'] #TODO: Add more color noises
        _LEN = _AUDIO.shape[0]
        ...
        if os.path.exists(_NOISE):
            ...
            if noise.shape[0] < _LEN:
                logging.warning('The noise file lenght is shorter than the audio file, repeting the noise file to fit the audio file...')
            ...
            if _CHNS!=0:
                if len(noise.shape) < 2:
                    ...
                    logging.error('The noise file should have one channel or a same number of channel as the input...')
                    raise ImportError('The noise file should have one channel or a same number of channel as the input...')
