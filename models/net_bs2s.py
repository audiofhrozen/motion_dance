...
class Dancer(chainer.Chain):
    ...
    def __call__(self, variables):
        ...
        for i in range(in_audio.shape[1]):
            state, y = self.forward(state, context, in_audio[:,i])
    ...
    def forward(self, state, h1, h):
        ...
        dc2, dh2 = self.dec_lstm2(state['dc2'], state['dh2'], dh1+eh3)
        dc3, dh3 = self.dec_lstm3(state['dc3'], state['dh3'], dh2+eh2)
        h = act(self.out_signal(dh3+eh1))
        new_state = dict()
        for key in state:
